...
class ImgDataset(torch.utils.data.Dataset):
    def __init__(self, flist, input_size, mask_rates=None, mask_path=None, augment=True, training=True, test_mask_path=None):
        ...
        ##250608##
        if training:
            self.custom_mask_list = []
            with open(mask_path) as f:
                for line in f:
                    self.custom_mask_list.append(line.strip())
            self.custom_mask_list = sorted(self.custom_mask_list, key=lambda x: x.split('/')[-1])
        else:
    ...
    def load_mask(self, img, index):
        ...
        # 测试阶段直接读取 self.mask_list
            mask = cv2.imread(self.mask_list[index], cv2.IMREAD_GRAYSCALE)
        else:
            # 训练阶段从自定义 mask 列表中随机选一个
            mask_index = random.randint(0, len(self.custom_mask_list) - 1)
            mask = cv2.imread(self.custom_mask_list[mask_index], cv2.IMREAD_GRAYSCALE)
...
class DynamicDataset(torch.utils.data.Dataset):
    ...
    def load_mask(self, img, index):
        ...
        if not self.training:
            ...
            mask_path = self.custom_mask_list[index % len(self.custom_mask_list)]

...
def _strip_list_item(x: str) -> str:
    """Clean one line from a txt list."""
    return x.strip().strip('"').strip("'")


def _resolve_list_path(path_str: str, list_file: str = None) -> str:
    """
    Resolve paths read from txt lists.

    Priority for relative paths:
      1) relative to current working directory, which matches most training/inference commands;
      2) relative to the txt list file directory, useful when lists are moved with data.
    The returned path is always absolute, even if the file does not exist, so later error messages
    print the exact resolved location.
    """
    p = _strip_list_item(path_str)
    if os.path.isabs(p):
    ...
    if list_file is not None:
        cand_list = os.path.abspath(os.path.join(os.path.dirname(os.path.abspath(list_file)), p))
        if os.path.exists(cand_list):
            return cand_list

    return cand_cwd


def _read_path_list(list_file: str, allow_empty: bool = False):
    list_file = _resolve_list_path(list_file)
    if not os.path.isfile(list_file):
        raise FileNotFoundError(f"list 文件不存在: {list_file}")
    items = []
    with open(list_file, 'r', encoding='utf-8') as f:
        for ln in f:
            s = _strip_list_item(ln)
            if not s or s.lstrip().startswith('#'):
                continue
            items.append(_resolve_list_path(s, list_file=list_file))
    if (not allow_empty) and len(items) == 0:
        raise RuntimeError(f"list 文件为空: {list_file}")
    return items
...
class ContinuousEdgeLineDatasetMask(Dataset):
    """
    1) line_path为pkls_list线框清单，不提供则报错；清单内支持相对路径/绝对路径
    2) 修复匹配规则：
    ...
    def __init__(self, pt_dataset, mask_path=None, test_mask_path=None, is_train=False,
        ...
        self.pt_dataset = _resolve_list_path(pt_dataset)

        # 读取图像清单：允许清单内为相对路径，但内部统一转成绝对路径。
        self.image_id_list = _read_path_list(self.pt_dataset)

        # 训练 / 测试掩码列表：允许相对路径，内部统一转成绝对路径。
        mask_list_file = mask_path if is_train else test_mask_path
        if mask_list_file is None:
            raise ValueError("必须提供 mask_path/test_mask_path")

        self.mask_list = sorted(
            _read_path_list(mask_list_file),
            key=_natural_key
        )

        # 可选：把掩码清单复制填满到与图片数相同的长度，仍然保留 load_mask 的取模兜底。
        ...
        self.line_path = _resolve_list_path(line_path) if line_path is not None else None
        self._lmap_cache = {}
        ...
        if self.line_path is not None:
            ...
            pkl_paths = _read_path_list(self.line_path)
            ...
            for p in pkl_paths:
                if not p.lower().endswith('.pkl'):
                    continue
                # p 已经在 _read_path_list 中解析为绝对路径；这里仅检查文件是否存在。

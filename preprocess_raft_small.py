#!/usr/bin/env python3
# preprocess_raft.py
# High-throughput RAFT-small preprocess with version-safe forward call + auto micro-batching.
...
# RAFT
# -----------------------------
def load_raft_small(device: torch.device, weights_name: str = "DEFAULT"):
    """
    Load torchvision RAFT-small.
    We normalize manually; do NOT call weights.transforms() (it needs img1,img2).
    """
    try:
        ...
        raise RuntimeError(
            ...
            "Upgrade torchvision to a version that provides torchvision.models.optical_flow.raft_small()."
        ) from e

    weights = None
    if weights_name and weights_name.upper() != "NONE":
        weights = getattr(Raft_Small_Weights, weights_name.upper(), Raft_Small_Weights.DEFAULT)
...
# -----------------------------
def main():
    parser = argparse.ArgumentParser("RAFT-small preprocess (max throughput)")
    ...
    parser.add_argument("--batch_size", type=int, default=128, help="Try big; auto micro-batch will split on OOM")
    ...
    # model
    model = load_raft_small(device=device, weights_name=args.weights)
    if args.compile:
        try:
    ...
    def infer_with_auto_split(prev_list: List[torch.Tensor], curr_list: List[torch.Tensor]) -> torch.Tensor:
        ...
        def _run(x1, x2):
            return raft_infer_batch(model, x1, x2, iters=args.iters, amp=args.amp, device=device)

        try:
            return _run(prev_b, curr_b)
        ...
        for i in range(len(metas)):
            ...
            if args.no_iou:
                ...
                if prev_lines.size == 0:
                    ...
                    iou_score = mask_iou(prev_mask, curr_mask)
                is_valid = (iou_score > float(args.iou_thr))
            ...
            payload = dict(
                ...
                confidence_iou=float(iou_score),
                valid=bool(is_valid),

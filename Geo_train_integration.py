...
class MaPDatasetWrapper(Dataset):
    def __init__(self, dataset, seq_to_indices, npz_path_list=None, flow_path_list=None, logger=None, no_align=False,
                 no_global_ref=False, no_local_ref=False):
        self.dataset = dataset
        self.seq_to_indices = seq_to_indices
        self.npz_path_list = npz_path_list
        self.flow_path_list = flow_path_list
        self.logger = logger
    ...
    def _warp_geometry_by_flow(self, data_dict, flow_hw2, shape):
        h, w = shape
        ...
        x_tgt = np.clip(np.rint(x_coords + flow[..., 0]), 0, w - 1).astype(np.int32)
        y_tgt = np.clip(np.rint(y_coords + flow[..., 1]), 0, h - 1).astype(np.int32)

        warped = {}
        for key in ['edge', 'line', 'mask']:
            src = np.asarray(data_dict[key], dtype=np.float32)
            dst = np.zeros((h, w), dtype=np.float32)
            dst[y_tgt, x_tgt] = np.maximum(dst[y_tgt, x_tgt], src)
            warped[key] = dst
        return warped

    def _select_alignment_npz(self, homo_path, flow_path):
        selected = None
        selected_conf = 0.0
        ...
        def _read_npz(path):
            ...
            try:
                ...
                return {
                    ...
                    'score': float(d.get('confidence_iou', 0.0)),
                    'homography': d['homography'] if 'homography' in d else None,
                    'flow': d['flow'] if 'flow' in d else None,
                }
            except Exception:
                return None

        homo_data = _read_npz(homo_path)
        flow_data = _read_npz(flow_path)

        threshold = float(getattr(self.dataset, 'integration_conf_threshold', 0.0))

        homo_score = homo_data['score'] if homo_data else -1.0
        flow_score = flow_data['score'] if flow_data else -1.0
        best_score = max(homo_score, flow_score)
        if best_score <= threshold:
            return selected, selected_conf, selected_valid

        if flow_score > homo_score:
            selected = ('flow', flow_data)
            selected_conf = flow_score
            selected_valid = bool(flow_data and flow_data['valid'] and flow_data['flow'] is not None)
        else:
            selected = ('homo', homo_data)
            selected_conf = homo_score
            selected_valid = bool(homo_data and homo_data['valid'] and homo_data['homography'] is not None)
        ...
        if self.no_local_ref:
            ...
            if self.no_align:
                ...
                try:
                    ...
                    pass
            elif (self.npz_path_list is not None) or (self.flow_path_list is not None):
                try:
                    homo_path = self.npz_path_list[idx] if self.npz_path_list is not None else None
                    flow_path = self.flow_path_list[idx] if self.flow_path_list is not None else None
                    selected, current_conf, selected_valid = self._select_alignment_npz(homo_path, flow_path)
                    npz_ok = 1.0
                    warp_valid = 1.0 if selected_valid else 0.0

                    if warp_valid > 0.5 and current_conf > 0.0:
                        prev_geo = self._load_raw_geometry(info['prev_idx'])
                        if selected[0] == 'flow':
                            warped_raw = self._warp_geometry_by_flow(
                                prev_geo, selected[1]['flow'],
                                (self.dataset.image_size, self.dataset.image_size)
                            )
                        else:
                            warped_raw = self._warp_geometry(
                                prev_geo, selected[1]['homography'],
                                (self.dataset.image_size, self.dataset.image_size)
                            )
...
def build_datasets_and_loader(opts, logger, train_npz_list=None, train_flow_list=None):
    ...
    base_dataset.integration_conf_threshold = float(getattr(opts, "integration_conf_threshold", 0.0))
    
    train_wrapper = MaPDatasetWrapper(
        base_dataset, 
        base_dataset.seq_to_indices, 
        npz_path_list=train_npz_list, 
        flow_path_list=train_flow_list,
        logger=logger,
...
def evaluate_sequence(model, val_dataset, seq_to_ref, device, logger, amp, opts,
                      writer=None, epoch=0, val_npz_list=None, val_flow_list=None):
    """
    ...
    def flow_warp_tensor(tensor_map, flow_hw2):
        t = tensor_map.float()
        if t.dim() == 3:
            t = t.unsqueeze(0)
        np_map = t.squeeze(0).permute(1, 2, 0).contiguous().cpu().numpy()
        ...
        if np_map.ndim == 2:
            np_map = np_map[..., None]
        dst = np.zeros_like(np_map, dtype=np.float32)
        for c in range(np_map.shape[2]):
            dst[..., c][y_tgt, x_tgt] = np.maximum(dst[..., c][y_tgt, x_tgt], np_map[..., c])

        warped_t = torch.from_numpy(dst).permute(2, 0, 1).unsqueeze(0)
        return warped_t.to(device=device, dtype=torch.float32)

    def select_alignment_npz(homo_path, flow_path):
        def _read_npz(path):
            if not path:
                return None
            try:
        ...
        h_data = _read_npz(homo_path)
        f_data = _read_npz(flow_path)
        thr = float(getattr(opts, 'integration_conf_threshold', 0.0))

        h_score = h_data['score'] if h_data else -1.0
        f_score = f_data['score'] if f_data else -1.0
        if max(h_score, f_score) <= thr:
            return None, 0.0, False

        if f_score > h_score:
            valid = bool(f_data and f_data['valid'] and f_data['flow'] is not None)
            return ('flow', f_data), f_score, valid

        valid = bool(h_data and h_data['valid'] and h_data['homography'] is not None)
        return ('homo', h_data), h_score, valid
        ...
        for seq_idx, (sid, info) in enumerate(fixed_meta.items()):
            ...
            try:
                ...
                if getattr(opts, "no_local_ref", False):
                    ...
                    warp_valid = True
                    local_conf = 1.0 # 强制全通
                elif (p_idx >= 0) and ((val_npz_list is not None) or (val_flow_list is not None)):
                    homo_path = val_npz_list[t_idx] if val_npz_list is not None else None
                    flow_path = val_flow_list[t_idx] if val_flow_list is not None else None
                    selected, local_conf, warp_valid = select_alignment_npz(homo_path, flow_path)
                ...
                if warp_valid and (p_idx >= 0):
                    try:
                        if is_no_align:
                            ...
                            if selected is not None and selected[0] == 'flow':
                                l_edge = flow_warp_tensor(prev_edge_gt, selected[1]['flow'])
                                l_line = flow_warp_tensor(prev_line_gt, selected[1]['flow'])
                            else:
                                H_mat = selected[1]['homography'] if selected is not None else np.eye(3, dtype=np.float32)
...
def main_worker(opts):
    ...
    def combine_datasets_from_yaml(cfg, mode='train'):
        ...
        for ds_key in ['dataset1', 'dataset2']:
            ...
            flow_list = read_list_file(ds.get(f"{prefix}flows_list"))
            if not img_list: continue
            seq_groups = collections.defaultdict(lambda: {'img': [], 'pkl': [], 'npz': [], 'flow': []})
            has_npz = (len(npz_list) == len(img_list))
            has_flow = (len(flow_list) == len(img_list))
            for i, img_path in enumerate(img_list):
                ...
                if has_flow: seq_groups[sid]['flow'].append(flow_list[i])
        ...
        final_img, final_pkl, final_npz, final_flow = [], [], [], []
        for d in dataset_info_list:
             for sid in sorted(list(d['seq_groups'].keys())):
                ...
                final_flow.extend(group['flow'])
        return final_img, final_pkl, final_npz, final_flow
    ...
    if getattr(opts, 'yaml_cfg', None):
        train_img, train_pkl, train_npz, train_flow = combine_datasets_from_yaml(opts.yaml_cfg, 'train')
        val_img, val_pkl, val_npz, val_flow = combine_datasets_from_yaml(opts.yaml_cfg, 'val')
        ...
        train_flow_list = train_flow
        val_flow_list = val_flow
    else:
        ...
        train_flow_list = opts.train_flow_list
        val_flow_list = opts.val_flow_list
    ...
    train_loader, val_dataset, train_sampler, train_base_ds = build_datasets_and_loader(
        opts, logger, train_npz_list=train_npz_list, train_flow_list=train_flow_list
    )
    ...
    for epoch in range(1, opts.train_epoch + 1):
        ...
        if ((epoch % opts.val_freq == 0) or (epoch == opts.train_epoch)):
            val_loss = evaluate_sequence(
                ...
                opts.AMP, opts, writer=writer, epoch=epoch, val_npz_list=val_npz_list, val_flow_list=val_flow_list
            )
            if is_main:
...
if __name__ == "__main__":
    ...
    parser.add_argument("--train_flow_list", type=str, nargs='+', default=None)
    parser.add_argument("--val_flow_list", type=str, nargs='+', default=None)
    parser.add_argument("--integration_conf_threshold", type=float, default=0.0)

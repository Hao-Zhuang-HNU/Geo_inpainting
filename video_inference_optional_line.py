...
def model_supports_line_guidance(model):
    """Return whether the loaded InpaintGenerator.forward accepts line_guidance."""
    try:
        return 'line_guidance' in inspect.signature(model.forward).parameters
    except (TypeError, ValueError, AttributeError):
        return False


def run_inpaint_generator(model, selected_imgs, selected_pred_flows_bi,
                          selected_masks, selected_update_masks, l_t,
                          selected_lines=None, use_line_guidance=False):
    """Run inpainting with optional line guidance.

    Important behavior:
    - If --line is not provided, call the model exactly like vanilla ProPainter.
    - If --line is provided and the model supports line_guidance, pass it.
    - If --line is provided but the loaded model does not support line_guidance,
      ignore the line maps instead of crashing.
    """
    if selected_lines is not None and use_line_guidance:
        return model(selected_imgs, selected_pred_flows_bi, selected_masks,
                     selected_update_masks, l_t, line_guidance=selected_lines)
    return model(selected_imgs, selected_pred_flows_bi, selected_masks,
...
if __name__ == '__main__':
    ...
    model.eval()
    model_can_use_line_guidance = model_supports_line_guidance(model)

    if args.line is None:
        print('[INFO] No --line provided. Running vanilla ProPainter inference without line guidance.')
    elif model_can_use_line_guidance:
        print('[INFO] --line provided and current InpaintGenerator supports line_guidance.')
    else:
        print('[WARN] --line provided, but current InpaintGenerator.forward does not support line_guidance. '
              'Line maps will be ignored.')
    ...
    for f in tqdm(range(0, video_length, neighbor_stride)):
        ...
        with torch.no_grad():
            ...
            pred_img = run_inpaint_generator(
                model,
                selected_imgs,
                ...
                use_line_guidance=model_can_use_line_guidance
            )

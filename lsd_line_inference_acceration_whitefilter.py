...
def build_white_mask(img_bgr, args):
    """
    Detect pure-white filled mask areas from the input image.
    This is intended for white-mask object-removal inputs where masked pixels are
    filled by (255, 255, 255). JPEG compression is tolerated by white_thresh.
    """
    thresh = int(np.clip(getattr(args, 'white_thresh', 245), 0, 255))
    # BGR image: white if all three channels are close to white.
    white = np.all(img_bgr >= thresh, axis=2).astype(np.uint8)

    k = int(getattr(args, 'white_dilate', 0) or 0)
    if k > 1:
        if k % 2 == 0:
            k += 1
...
def line_white_overlap_ratio(line_xyxy, white_mask, band_width=5):
    """
    Compute how much of a line support band overlaps with detected white-mask pixels.
    line_xyxy is [x1, y1, x2, y2].
    """
    ...
    thickness = int(max(1, band_width))
    cv2.line(line_band, p1, p2, 1, thickness, cv2.LINE_AA)
    band = line_band > 0
    total = int(band.sum())
    if total <= 0:
        return 0.0, 0
    overlap = int(np.logical_and(band, white_mask).sum())
    return float(overlap) / float(total), overlap


def should_drop_by_white_overlap(line_xyxy, white_mask, args):
    """
    Drop mask-boundary or mask-interior line segments.
    A line is removed if its support band intersects too many pure-white pixels.
    """
    ratio, overlap = line_white_overlap_ratio(
        line_xyxy,
        white_mask,
        band_width=int(getattr(args, 'white_band_width', 5) or 5),
    )
    min_pixels = int(getattr(args, 'white_min_pixels', 5) or 5)
    ratio_thr = float(getattr(args, 'white_overlap_ratio', 0.25) or 0.25)
    return overlap >= min_pixels and ratio >= ratio_thr
...
def detect_lsd_lines(img_bgr, args):
    ...
    white_mask = build_white_mask(img_bgr, args) if getattr(args, 'white_filter', True) else None
    ...
    for line in raw_lines.reshape(-1, 4):
        ...
        if args.max_length and float(args.max_length) > 0 and length > float(args.max_length):
            continue
        if white_mask is not None and should_drop_by_white_overlap([x1, y1, x2, y2], white_mask, args):
            continue
...
def parse_args():
    ...
    # White-mask boundary suppression.
    parser.add_argument('--no_white_filter', dest='white_filter', action='store_false',
                        help='Disable white-mask overlap filtering. By default, lines that overlap many pure-white pixels are removed.')
    parser.set_defaults(white_filter=True)
    parser.add_argument('--white_thresh', type=int, default=245,
                        help='Pixels with all RGB/BGR channels >= this value are treated as white mask. Default: 245')
    parser.add_argument('--white_dilate', type=int, default=2,
                        help='Dilate detected white mask before filtering. Use odd kernel size; 0/1 disables. Default: 3')
    parser.add_argument('--white_band_width', type=int, default=5,
                        help='Line support band width for white-overlap test. Default: 5')
    parser.add_argument('--white_overlap_ratio', type=float, default=0.25,
                        help='Drop a line if white pixels occupy at least this ratio of its support band. Default: 0.25')
    parser.add_argument('--white_min_pixels', type=int, default=5,
                        help='Minimum white-overlap pixels required before dropping a line. Default: 5')
...
def main():
    ...
    print(f'[INFO] white_filter            : {args.white_filter}')
    if args.white_filter:
        print(f'[INFO] white_thresh            : {args.white_thresh}')
        print(f'[INFO] white_dilate            : {args.white_dilate}')
        print(f'[INFO] white_band_width        : {args.white_band_width}')
        print(f'[INFO] white_overlap_ratio     : {args.white_overlap_ratio}')

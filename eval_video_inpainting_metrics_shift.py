...
def main():
    ...
    parser.add_argument("--mask_shift", type=int, default=0,
                        help="mask序列偏移量。1表示mask往后错1位（mask[i]对应img[i-1]）")
    ...
    frame_indices = list(range(N))  # original image/gt indices before temporal shift

    # 2) mask：按自然排序 index 对齐
    mask_frames: List[Optional[Path]] = [None] * N
    mask_indices: List[Optional[int]] = [None] * N
    if mask_root is not None:
    ...
    # 3) real temporal mask shift
    # mask_shift = 1 means: frame2 uses mask1, frame3 uses mask2, ...
    # i.e., skip the first image frame and drop the last mask.
    if args.mask_shift != 0:
        shift = int(args.mask_shift)
        if abs(shift) >= N:
            raise RuntimeError(f"abs(mask_shift)={abs(shift)} must be smaller than frame count N={N}")

        if shift > 0:
            pred_frames = pred_frames[shift:]
            gt_frames = gt_frames[shift:]
            frame_indices = frame_indices[shift:]
            mask_frames = mask_frames[:-shift]
            mask_indices = mask_indices[:-shift]
            print(f"[INFO] Applied mask_shift={shift}: frame[{shift}:] <-> mask[:-{shift}]")
        else:
            shift_abs = abs(shift)
            pred_frames = pred_frames[:-shift_abs]
            gt_frames = gt_frames[:-shift_abs]
            frame_indices = frame_indices[:-shift_abs]
            mask_frames = mask_frames[shift_abs:]
            mask_indices = mask_indices[shift_abs:]
            print(f"[INFO] Applied mask_shift={shift}: frame[:-{shift_abs}] <-> mask[{shift_abs}:]")
        ...
        print(f"[INFO] After mask_shift: eval_frames={N}")
    ...
    for idx, (p, g, m) in enumerate(tqdm(list(zip(pred_frames, gt_frames, mask_frames)), desc="Frame metrics", total=N)):
        ...
        if args.debug:
            debug_records.append({
                ...
                "hole_pixels": int(mask_ratio * pred.shape[0] * pred.shape[1]) if not np.isnan(mask_ratio) else None,
                ...
                "hole_psnr_gt_all": (
                    ...
                    if (not np.isnan(psnr_h) and not np.isnan(psnr_all))
                    else None
                ),
                "mask_shift": args.mask_shift,
                "aligned_pair": f"img[{frame_indices[idx]}] <-> mask[{mask_indices[idx]}]",
            })
    ...
    results = {
        "count_frames": N,
        "mask_shift": int(args.mask_shift),
    ...
    if args.debug:
        ...
        mismatch_gt_mask = sum([
            (x["gt_mask_same_name"] is False)
            for x in debug_records
        ])

        hole_gt_all_count = sum([
            (x["hole_psnr_gt_all"] is True)
            for x in debug_records
        ])
        ...
        with open(debug_txt, "w", encoding="utf-8") as f:
            ...
            f.write(f"mask_shift: {int(args.mask_shift)}\n")
            if len(debug_records) > 0:
            ...
            f.write(f"PSNR_hole_gt_PSNR_all_count: {hole_gt_all_count}\n")

            valid_ratios = [
                x["mask_ratio"] for x in debug_records
                if x["mask_ratio"] == x["mask_ratio"]
            ]
            ...
            f.write("2. mask与gt/pred未严格对应（index对齐错误）。\n")
            f.write("3. mask前景/背景取反。\n")
            f.write("4. resize后mask边界发生偏移。\n")
            f.write("5. pred/gt存在排序差异（例如0001.png与1.png）。\n")

...
def dilate_binary_mask(mask: np.ndarray, dilate_px: int) -> np.ndarray:
    """
    Dilate binary foreground mask by approximately dilate_px pixels.
    Example:
      --mask_dilate 3 means foreground expands outward by about 3 pixels.
      --mask_dilate 0 disables dilation.
    """
...
def apply_mask_to_bgra_alpha(bgra: np.ndarray, mask: np.ndarray, feather: int = 0) -> np.ndarray:
    """Replace BGRA alpha channel using final mask."""
...
def remove_background_rembg(img_path: Path, args: argparse.Namespace) -> Tuple[np.ndarray, np.ndarray]:
    """
    Deep-learning backend using rembg.
    Return:
      bgra: BGRA image
      mask: binary uint8 mask, 0 background / 255 foreground
    """
...
def remove_background_grabcut(image: np.ndarray, args: argparse.Namespace) -> Tuple[np.ndarray, np.ndarray]:
    if image is None:
...
def process_one_image(
    ...
    try:
        if args.resolved_backend == "rembg":
            ...
            bgra, mask = remove_background_grabcut(image, args)
        else:
            return False, f"Unknown backend: {args.resolved_backend}"

        # New parameter: mask dilation.
        # Default --mask_dilate 3 expands foreground by about 3 pixels.
...
def process_one_image_worker(
    ...
    cv2.setNumThreads(1)
    return process_one_image(img_path, input_root, output_root, mask_root, args)

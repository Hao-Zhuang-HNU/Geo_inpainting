...
def get_inpainting_metrics(src, tgt, logger, fid_test=True):
    ...
    input_paths = sorted(glob(os.path.join(src, '**', '*.*'), recursive=True))
    output_paths = sorted(glob(os.path.join(tgt, '**', '*.*'), recursive=True))
    print(f"[Info] reading result images {len(input_paths)}  from {src}")
    print(f"[Info] reading GT images {len(output_paths)}  from {tgt}")
    ## 250608 ##

...
class EdgeLineGPT256RelBCE(nn.Module):
    ...
    def extract_reference_features(self, 
        ...
        if global_img is not None or global_edge is not None:
            ...
            g_feat_flat = g_feat.flatten(2).transpose(1, 2)
            g_feat_final = g_feat_flat + self.type_emb_global
        ...
        if local_img is not None or local_edge is not None:
            ...
            # No Pooling
            l_feat_flat = l_feat.flatten(2).transpose(1, 2)
            l_feat_final = l_feat_flat + self.type_emb_local

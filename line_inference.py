...
def resolve_worker_count(requested, total, default_cap=None):
    """
    requested=0 means auto: use as many CPU cores as practical.
    A cap can be passed when too many workers would hurt GPU-bound workloads.
    """
...
def run_hawp_preprocess(opts, img_paths, image_base_root, pkl_root, pkl_list_path):
    ...
    if total_imgs > 0:
        ...
        n_workers = resolve_worker_count(opts.hawp_num_workers, total_imgs)
...
def _overlay_one_task(task):
    ...
    # Avoid OpenCV internal thread oversubscription inside multiprocessing workers.
    cv2.setNumThreads(0)
    ...
    if img is None:
        return i, None, str(mask_path), f'Failed to read image: {img_path}'
    ...
    try:
        ...
        if mode in ('white', 'fill'):
            ...
            return i, None, str(mask_path), f'Unsupported overlay mode: {mode}'

        out_path = make_output_path(img_path, img_base_root, out_root)
        ok = cv2.imwrite(str(out_path), out)
        if not ok:
            return i, None, str(mask_path), f'Failed to write output: {out_path}'
        return i, str(out_path), str(mask_path), None
    except Exception as e:
        return i, None, str(mask_path), str(e)
...
def apply_masks_to_images_from_lists(
    ...
    num_workers=0 表示自动使用可用 CPU 核心数；由于 overlay 主要是 imread/resize/write，
    多进程 CPU 通常比逐张串行或 GPU 上传/下载更快、更稳定。
    """
    ...
    overlay_workers = resolve_worker_count(num_workers, n)
    ...
    print(f'[PRE] Mask overlay workers      : {overlay_workers}')
    ...
    results = [None] * n
    if overlay_workers <= 1 or n <= 1:
        for task in tqdm(tasks, desc='Overlay masks', unit='img', dynamic_ncols=True):
            i, out_path, mask_path, err = _overlay_one_task(task)
            if err:
                raise RuntimeError(err)
            results[i] = (Path(out_path), Path(mask_path))
    else:
        # fork 在 Linux 容器中启动开销低；spawn 在部分环境更稳。这里优先 fork，失败再退回默认上下文。
        try:
            ctx = mp.get_context('fork')
        except ValueError:
            ctx = mp.get_context()
        with ctx.Pool(processes=overlay_workers, maxtasksperchild=200) as pool:
            for i, out_path, mask_path, err in tqdm(
                pool.imap_unordered(_overlay_one_task, tasks, chunksize=max(1, n // (overlay_workers * 8))),
                total=n,
                desc='Overlay masks',
                unit='img',
                dynamic_ncols=True,
    ...
    written_paths = [x[0] for x in results if x is not None]
    used_mask_paths = [x[1] for x in results if x is not None]

    if len(written_paths) != n:
        raise RuntimeError(f'Only wrote {len(written_paths)} masked images, expected {n}.')
...
def preprocess_inputs(opts):
    ...
    if not opts.no_mask_overlay:
        ...
        masked_list, masked_paths, used_mask_paths_after_overlay = apply_masks_to_images_from_lists(
            ...
            repeat_masks=True,
            num_workers=opts.overlay_num_workers,
        )
...
def build_model(opts):
    ...
    if getattr(opts, 'compile_model', False):
        try:
            print('[ACCEL] torch.compile enabled for line model.')
            model = torch.compile(model)
        except Exception as e:
            print(f'[WARN] torch.compile failed; continue without compile. Error: {e}')

    return model


def save_pred(prob, gt, mask, out_file=None, binary_threshold=None, write_file=True):
    """
    Merge predicted masked region with known-region GT on GPU and optionally save visualization.
    Old version moved merged output to CPU and then back to CUDA; this version keeps the returned
    reference tensor on GPU, which is noticeably faster in sequential inference.
    """
    if prob.dim() == 2:
        pred = prob.unsqueeze(0).unsqueeze(0)
    elif prob.dim() == 3:
        pred = prob.unsqueeze(0)
    else:
        pred = prob

    gt4 = _ensure_4d(gt).to(device=pred.device, dtype=pred.dtype, non_blocking=True)
    mask4 = _ensure_4d(mask).to(device=pred.device, dtype=pred.dtype, non_blocking=True)

    if binary_threshold is not None:
        pred = (pred >= float(binary_threshold)).to(dtype=gt4.dtype)

    merged_t = pred * mask4 + gt4 * (1 - mask4)
    merged_t = merged_t.detach()

    if write_file and out_file:
        merged_np = merged_t.squeeze().float().clamp(0, 1).cpu().numpy()
        vis = (np.stack([merged_np] * 3, axis=-1) * 255).astype(np.uint8)
        cv2.imwrite(out_file, vis[:, :, ::-1])

    return merged_t


def _autocast_context(opts):
    if getattr(opts, 'amp_inference', False):
        return torch.cuda.amp.autocast(dtype=torch.float16)
    return contextlib.nullcontext()


def _to_cuda_4d(t):
    return _ensure_4d(t).cuda(non_blocking=True)


def _iter_dataset_items(dataset, opts):
    workers = int(getattr(opts, 'data_num_workers', 0) or 0)
    if workers <= 0:
    ...
    loader = DataLoader(
        ...
        prefetch_factor=max(2, int(getattr(opts, 'prefetch_factor', 2))),
    )
    for item in loader:
        yield item
...
def geo_inference(opts):
    ...
    torch.backends.cudnn.benchmark = True
    if getattr(opts, 'enable_tf32', True):
    ...
    print(f'[ACCEL] amp_inference={opts.amp_inference}, data_num_workers={opts.data_num_workers}, '
          f'skip_save_edge={opts.skip_save_edge}, tf32={opts.enable_tf32}')

    item_iter = _iter_dataset_items(dataset, opts)
    ...
    img0 = _to_cuda_4d(first['img'])
    mask0 = _to_cuda_4d(first['mask'])
    gt_e0 = _to_cuda_4d(first['edge'])
    gt_l0 = _to_cuda_4d(first['line'])

    with torch.inference_mode(), _autocast_context(opts):
    ...
    pred_edge_prev = save_pred(
        edge0[0],
        gt_e0,
        mask0,
        None if opts.skip_save_edge else os.path.join(opts.save_url, 'edge', name0),
        write_file=not opts.skip_save_edge,
    )
    ...
    for item in tqdm(item_iter, total=max(0, len(dataset) - 1), desc='Line inference', unit='frame', dynamic_ncols=True):
        name = os.path.basename(item['name'][0] if isinstance(item['name'], list) else item['name'])
        if not name.lower().endswith(('.png', '.jpg', '.jpeg')):
            name += '.png'

        img = _to_cuda_4d(item['img'])
        mask = _to_cuda_4d(item['mask'])
        gt_e = _to_cuda_4d(item['edge'])
        gt_l = _to_cuda_4d(item['line'])

        local_edge = dilate_tensor(pred_edge_prev, opts.ref_dilate)
        local_line = dilate_tensor(pred_line_prev, opts.ref_dilate)
        ...
        pred_edge_prev = save_pred(
            ...
            gt_e,
            mask,
            None if opts.skip_save_edge else os.path.join(opts.save_url, 'edge', name),
            write_file=not opts.skip_save_edge,
        )
        pred_line_prev = save_pred(
            line_pred[0],
...
def build_argparser():
    ...
    parser.add_argument('--hawp_num_workers', type=int, default=16,
                        help='Number of HAWP worker processes. Default: 16; use 0 for auto CPU-core count capped by image count.')
    ...
    parser.add_argument('--overlay_num_workers', type=int, default=0,
                        help='Mask overlay worker processes. 0 means auto use available CPU cores. Default: 0')

    # Inference acceleration options.
    parser.add_argument('--amp_inference', action='store_true',
                        help='Use CUDA fp16 autocast during line inference. Faster on RTX 3090, with tiny numerical differences possible.')
    parser.add_argument('--data_num_workers', type=int, default=0,
                        help='DataLoader workers for image/mask/pkl prefetch during sequential inference. Default: 0')
    parser.add_argument('--prefetch_factor', type=int, default=2,
                        help='DataLoader prefetch factor when --data_num_workers > 0. Default: 2')
    parser.add_argument('--skip_save_edge', action='store_true',
                        help='Do not write save_url/edge images. This reduces disk I/O when only line outputs are needed.')
    parser.add_argument('--no_tf32', dest='enable_tf32', action='store_false',
                        help='Disable TF32 matmul/cudnn acceleration.')
    parser.set_defaults(enable_tf32=True)
    parser.add_argument('--compile_model', action='store_true',
                        help='Try torch.compile(model). Usually only useful for long runs; first run has compile overhead.')
...
if __name__ == '__main__':
    ...
    # OpenCV internal threading + Python multiprocessing can oversubscribe CPU; keep workers explicit.
    cv2.setNumThreads(0)

...
def main():
    ...
    # --jump_image N:
    #   skip first N pred/gt images.
    #   Example: --jump_image 1 -> pred[1] / gt[1] <-> mask[0]
    #
    # --jump_mask N:
    #   skip first N gt/mask frames, following the jump_first logic.
    #   Example: --jump_mask 1 -> pred[0] <-> gt[1] <-> mask[1]
    #
    # If both are set, offsets are combined as:
    #   pred[i + jump_image] <-> gt[i + jump_image + jump_mask] <-> mask[i + jump_mask]
    ...
    if ji > 0 or jm > 0:
        ...
        print(f"[INFO] New lengths -> pred:{len(pred_frames)} gt:{len(gt_frames)} mask:{len(mask_frames)}")
    else:
    ...
    results = {
        ...
        "mask_shift": int(args.jump_image),
    ...
    if args.debug:
        ...
        with open(debug_txt, "w", encoding="utf-8") as f:
            ...
            f.write(f"mask_shift: {int(args.jump_image)}\n")
            if len(debug_records) > 0:

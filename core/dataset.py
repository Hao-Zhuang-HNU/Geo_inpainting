...
def _is_image_file(path):
    return str(path).lower().endswith(('.png', '.jpg', '.jpeg', '.bmp', '.tif', '.tiff', '.webp'))


def load_line_image_to_pil(line_path, size, force_rgb=False):
    """Load a pre-rendered line image.

    If the input is RGB and force_rgb=True, preserve all three channels.
    This is required for 3-channel reliability-aware fields:
      R=center, G=soft, B=reliability.
    Otherwise return grayscale L for legacy single-channel LinePro.
    """
    W, H = int(size[0]), int(size[1])
    if force_rgb:
        img = cv2.imread(str(line_path), cv2.IMREAD_UNCHANGED)
        if img is None:
            raise FileNotFoundError(f'Cannot read line image: {line_path}')
        if img.ndim == 2:
...
def load_lines_from_pkl_or_torch(pkl_path):
    """Load wireframe lines from normal pickle or torch-saved pickle-like files.

    Some HAWP/LSM caches are saved through torch.save(), which raises:
    "A load persistent id instruction was encountered..." when read by
    pickle.load(). This fallback prevents training from crashing.
    """
    try:
        return load_lines_from_pkl(pkl_path)
    except Exception as e:
        msg = str(e)
        if ('persistent id' not in msg) and ('invalid load key' not in msg) and ('pickle' not in e.__class__.__name__.lower()):
            raise

    data = torch.load(pkl_path, map_location='cpu')
    if isinstance(data, dict):
        for key in ['lines', 'line', 'wireframe', 'pred_lines', 'line_segments']:
            if key in data:
                data = data[key]
                break
...
class TrainDataset(torch.utils.data.Dataset):
    def __init__(self, args: dict):
        ...
        # Whether pre-rendered line images should be loaded as RGB.
        # Required for 3-channel reliability-aware fields:
        #   R=center, G=soft, B=reliability.
        # Defaults to False to preserve legacy single-channel LinePro behavior.
        self.line_force_rgb = bool(args.get('line_force_rgb', False))
    ...
    def __getitem__(self, index):
        ...
        for idx in selected_index:
            ...
            if self.use_line:
                ...
                # Fallback when line_root is a folder. Prefer image soft-line maps,
                # then legacy pkl wireframes.
                if line_path is None and self.line_root is not None:
                    candidates = [
                        os.path.join(self.line_root, video_name, line_stem + ext)
                        for ext in ['.png', '.jpg', '.jpeg', '.bmp', '.tif', '.tiff', '.webp', '.pkl']
                    ]
                ...
                if line_path is not None and os.path.exists(line_path):
                    if _is_image_file(line_path):
                        # Precomputed reliability-aware soft-guidance PNG/JPG.
                        # It is already rendered in image coordinates.
                        line_img = load_line_image_to_pil(line_path, self.size, force_rgb=self.line_force_rgb)
                    else:
                        # Legacy pkl/torch-saved line segment format.
                        line_segments = load_lines_from_pkl_or_torch(line_path)
                        ...
                        if self.line_force_rgb:
                            line_img = line_img.convert('RGB')
                else:
                    if self.line_force_rgb:
                        line_img = Image.fromarray(np.zeros((self.h, self.w, 3), dtype=np.uint8), mode='RGB')
                    else:
